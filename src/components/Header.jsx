...
export function Header({ isSection2Visible }) {
  const [expandedLiIndex, setExpandedLiIndex] = useState(null);

  const handleMouseEnter = (index) => {
    setExpandedLiIndex(index);
  };

  const handleMouseLeave = () => {
    setExpandedLiIndex(null);
  };


  return (
    <header className={`flex px-10 fixed top-0 w-full ${isSection2Visible ? 'text-black' : 'text-slate-100'} z-30`}>
      ...
      <nav className="hidden lg:flex font-['Gotham Light'] text-[15px] font-semibold">
        <ul className="flex  [&>li>a]:mx-4 my-4  [&>li>a]:inline-block [&>li>a]:p-1 [&>li>]:relative">
          {['Vehículos', 'Energía', 'Carga', 'Descubrir', 'Tienda'].map((menuItem, index) => (
            <li key={index}  onMouseEnter={() => handleMouseEnter(index)}  onMouseLeave={handleMouseLeave} >
              <a href='./'>{menuItem}</a>
               {expandedLiIndex === index && (
                <div className="fixed inset-0  flex items-center justify-center backdrop-blur-sm">
                  <div className="w-full h-[24em] z-50 fixed top-0 left-0 h-screen flex justify-center items-center bg-white shadow-md p-4  opacity-100 scale-100 transform  animate__animated  animate__fadeInDown"onMouseLeave={handleMouseLeave} > 
                    {<MenuHeader/>}
                  </div>
                </div>
              )}
